...
def parse(html):
    count_s = 120
    brand = 1000
    br = ['Alfa Romeo','Dodge' ,'Kia','Nissan','Skoda','Audi','Fiat' ,'Lada (ВАЗ)','Opel','Subaru','BMW','Ford',
         'Lexus','Peugeot','Suzuki','Chevrolet','Geely','Mazda','Renault','Toyota','Chrysler','Honda','Mercedes-Benz',
         'Rover','Volkswagen','Citroen','Hyundai','Mitsubishi','SEAT','Volvo']
    for e in range(1, brand):
        sum_list = []
        soup = BeautifulSoup(get_html(html + f'[0][brand]={e}').text, 'html.parser')
        soup_find = soup.find('span', class_='dropdown-floatlabel__value').text
        if str(soup_find) in str(br):
            for i in range(1, count_s + 1):
                soup = BeautifulSoup(get_html(html + f'[{e}][brand]=8&page={i}').text, 'html.parser')
                lict_new = soup.findAll('div', class_='listing-item__wrap')
                for j in lict_new:
                    sum_list.append(int(j.find('div', class_='listing-item__price').text.replace('\xa0р.', '').replace(
                            '\u2009', '')))
                print('Обработано: ', i)
            else:
